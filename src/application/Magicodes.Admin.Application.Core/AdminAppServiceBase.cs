...
namespace Magicodes.Admin.Application.Core
{
    ...
    public abstract class AdminAppServiceBase : ApplicationService
    {
        /// <summary>
        /// </summary>
        protected AdminAppServiceBase() => LocalizationSourceName = LocalizationConsts.LocalizationSourceName;
        ...
        /// </summary>
        /// <returns></returns>
        protected virtual User GetCurrentUser() => AsyncHelper.RunSync(GetCurrentUserAsync);
        ...
        where TEntity : class, IEntity<TPrimaryKey>
    {
        ...
        /// <returns></returns>
        protected virtual async Task Import<TDto>(string filePath, Func<ICollection<TDto>, ICollection<TEntity>> func = null)
            where TDto : class, new()
        {
            ...
            if (result.HasError)
            {
                throw new UserFriendlyException(L("HasImportError"));
            }
            ...
            where TDto : class, new()
        {
            ...
            filePath = filePath.Replace(Path.GetFileName(filePath), $"{Path.GetFileNameWithoutExtension(filePath)}_{Path.GetExtension(filePath)}");
            filePath = GetFilePath(filePath);
        ...
        private string GetFilePath(string filePath)
        {
            ...
            {
                throw new UserFriendlyException(L("FileNotAllowedEmpty"));
            }

            if (!Path.IsPathRooted(filePath))
            {
                filePath = Path.Combine(AppFolders.TemplateFilePath, filePath);
            }
